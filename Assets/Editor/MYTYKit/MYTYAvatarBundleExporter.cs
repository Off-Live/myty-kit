...
namespace MYTYKit
{
    public class MYTYAvatarBundleExporter : EditorWindow
    {
        ...
        {
            var wnd = CreateInstance<MYTYAvatarBundleExporter>();
        ...
        void Export()
        {
            ...
            if (Directory.Exists(MYTYPath.PrefabPath))
            {
                Directory.Delete(MYTYPath.PrefabPath, true);
            }

            Directory.CreateDirectory(MYTYPath.PrefabPath);
            ...
            if (Directory.Exists(MYTYPath.BundlePath))
            {
                Directory.Delete(MYTYPath.BundlePath, true);
            }

            Directory.CreateDirectory(MYTYPath.BundlePath);
            ...
            using (ZipArchive zip = ZipFile.Open(MYTYPath.BundlePath+ "/"+ filename + ".zip", ZipArchiveMode.Create))
            {
        ...
        void PrepareLayoutAsset(AvatarSelector selector, List<string> assetName)
        {
            ...
            var path = AssetDatabase.GenerateUniqueAssetPath(MYTYPath.PrefabPath + "/refCamera.prefab");
            var savedCamera = PrefabUtility.SaveAsPrefabAsset(mainCamera.gameObject, path);
        
            AssetDatabase.CreateAsset(assetSCO, MYTYPath.PrefabPath + "/" + "DefaultLayoutAsset.asset");
            assetName.Add(path);
            assetName.Add(MYTYPath.PrefabPath + "/" + "DefaultLayoutAsset.asset");
        ...
        private bool PrepareConfigurator(MYTYAssetScriptableObject mytyAsset, List<string> assetName)
        {
            ...
            foreach(var template in selector.templates)
            {
                ...
                var newPath = MYTYPath.PrefabPath + "/" + template.instance.name+".asset";
            ...
            PrefabUtility.SaveAsPrefabAsset(copyGO, MYTYPath.PrefabPath+"/"+ MYTYPath.SelectorPrefabPath);
            DestroyImmediate(copyGO);
            assetName.Add(MYTYPath.PrefabPath + "/" + MYTYPath.SelectorPrefabPath);
            return true;
        }
        ...
        private bool PrepareController(MYTYAssetScriptableObject mytyAsset, List<string> assetName)
        {
            ...
            foreach (var elem  in rootCons)
            {
                ...
                var path = AssetDatabase.GenerateUniqueAssetPath(MYTYPath.PrefabPath + "/" + rootConGO.name + ".prefab");
        ...
        bool PrepareMotionAdapter(MYTYAssetScriptableObject mytyAsset, List<string> assetName)
        {
            ...
            foreach (var elem in motionAdapters)
            {
                ...
                var path = AssetDatabase.GenerateUniqueAssetPath(MYTYPath.PrefabPath + "/" + motionAdapterGo.name + ".prefab");
        ...
        void PrepareVersionInfo(List<string> assetName)
        {
            ...
            var writer = File.CreateText(MYTYPath.PrefabPath + "/VERSION.txt");
            writer.Write(versionStr);
            writer.Close();
            assetName.Add(MYTYPath.PrefabPath + "/VERSION.txt");
        }

        void PrepareEditorInfo(List<string> assetName)
        {
            var filepath = MYTYPath.PrefabPath + "/EditorInfo.txt";
        ...
        void PrepareMotionTemplateMapper(List<string> assetName)
        {
            ...
            var path = AssetDatabase.GenerateUniqueAssetPath(MYTYPath.PrefabPath + "/MotionTemplate.prefab");
        ...
        {
            var filepath = MYTYPath.PrefabPath + "/AvatarImportSig.txt";
            StreamWriter file = new(filepath); 
        ...
        {
            var filepath = MYTYPath.AssetPath + "/ARFaceData.asset";
            var arToggle = rootVisualElement.Q<Toggle>("CHKIncludeAR");
